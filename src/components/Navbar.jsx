    ...
    return (
        <nav className="flex flex-col md:flex-row bg-slate-900 items-center">
            <div className="md:w-1/2 flex items-center">
                ...
                <h1 className='text-xl font-semibold'>Tashdik-WEB</h1>
            </div>
            <ul className="flex justify-around w-full md:w-1/2">
                {
                    links.map(link => <Link className={pathName === link.path && 'text-green-500'} key={link.title} href={link.path}>{link.title}</Link>)
                }
            </ul>
        </nav>
    );
